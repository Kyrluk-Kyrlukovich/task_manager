<template>
  <div class="h-full w-full flex bg-slate-300" :class="classFontSize">
    ...
    <div v-show="modalAcceptedAction.isOpen" class="h-full w-full z-10 absolute" @click="closeAccept">
      <transition name="modalAccept">
        <div v-if="modalAcceptedAction.isOpen" @click.stop
             class="absolute p-5 pb-2 z-20 opacity-100 overflow-hidden left-[50%] top-[50%] translate-x-[-50%] translate-y-[-50%] rounded-[10px] shadow-[1px_3px_27px_8px_rgba(34,60,80,0.2)] bg-slate-200  h-[150px] w-[325px]">
    ...
    <transition name="modalError">
      <div v-show="errorAction.isError"
           class="absolute  p-8 z-20 opacity-100 overflow-hidden right-[2%] bottom-[2%] rounded-[10px] shadow-[1px_3px_27px_8px_rgba(34,60,80,0.2)] bg-slate-200  h-[125px] w-[325px]">
...
export default {
  ...
  data() {
    return {
      classFontSize: {
        firefox_font_size: navigator.userAgent.indexOf('Firefox') != -1,
        chrome_font_size: navigator.userAgent.indexOf('Chrome') != -1
      }
    };
  },
...
.modalError-enter-active {
  animation: open-modal-error 0.4s;
}

.modalError-leave-active {
  animation: open-modal-error 0.5s reverse;
}
