<template>
  <div class="flex h-[91%]" :class="[isListTask ? '' : 'justify-center']">
    <div
        class="px-6 flex gap-y-10 flex-wrap" :class="[isListTask ? 'w-9/12' : 'w-11/12']"
    >
      <CalendarTasks/>
    </div>
    <div v-if="isListTask" class="w-3/12 flex flex-wrap">
      <ChoosenDate class="h-[10%]"/>
      <ListTasks class="h-[80%]"/>
      <transition name="modalCreateTask">
        <div v-if="showModal"
             class="absolute overflow-hidden left-[50%] top-[50%] translate-x-[-50%] translate-y-[-50%] rounded-[10px] shadow-[1px_3px_27px_8px_rgba(34,60,80,0.2)] bg-slate-200 max-h-[475px] h-full max-w-[550px] w-full">
